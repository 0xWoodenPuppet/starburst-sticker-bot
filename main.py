...
try:
    with open("stickers.csv", mode="r", encoding="utf-8") as file:
        ...
        for row in reader:
            if len(row) == 2:
                # CHANGED: Triggers are stored in lowercase for case-insensitive matching
                all_triggers.append((row[0].strip().lower(), row[1].strip()))
...
# 🚀 Create the final dictionary from the sorted list.
TRIGGERS = {trigger: sticker_id for trigger, sticker_id in all_triggers}

# REMOVED: The TRIGGER_PATTERNS dictionary has been removed.
...
async def check_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Refactored to handle both message types cleanly
    if isinstance(update, Update) and update.message:
        if message.forward_origin or message.sender_chat:
            return
        user_id = message.from_user.id
    else:
        user_id = 0 # For channel posts
    ...
    # CHANGED: Using a simple, case-insensitive substring search instead of regex.
    lower_text = message.text.lower()
    for trigger in TRIGGERS:
        if trigger in lower_text:
            await message.reply_sticker(sticker=TRIGGERS[trigger])
...
async def get_trigger(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Saves the sticker and dynamically updates the in-memory trigger list."""
    global TRIGGERS, all_triggers # ADDED: To modify the live trigger lists

    trigger_text = update.message.text.strip().lower()
    sticker_id = context.user_data.get('new_sticker_id')

    # The duplicate check now checks against the live triggers dictionary
    ...
    # --- CHANGED: Logic to update live triggers without regex ---
    # 1. Add the new trigger to the main list in memory
    all_triggers.append((trigger_text, sticker_id))
    
    # 2. Re-sort the entire list to maintain specificity (longest first)
    all_triggers.sort(key=lambda item: len(item[0]), reverse=True)

    # 3. Rebuild the TRIGGERS dictionary in the new correct order
    TRIGGERS = {trigger: sticker_id for trigger, sticker_id in all_triggers}
    # --- END OF CHANGED LOGIC ---

    await update.message.reply_text(
        f"✅ Success! Trigger '{trigger_text}' has been saved and is now active.\n\n"
